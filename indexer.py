...
from utils import logger, ProgressTracker, split_into_batches
...
class DocumentIndexer:
    ...
    def _initialize_embeddings(self):
        ...
        try:
            ...
            logger.info(f"嵌入模型初始化成功: {Config.EMBEDDING_MODEL}")
    ...
    def _get_or_create_vectorstore(self) -> Optional[Chroma]:
        ...
        try:
            ...
            logger.info(f"向量数据库连接成功: {Config.CHROMA_DB_PATH}")
    ...
    def create_embeddings(self, documents: List[Document]) -> Optional[Chroma]:
        ...
        try:
            ...
            document_batches = split_into_batches(documents, batch_size)
            ...
            for batch in document_batches:
                try:
                    ...
                    logger.info(f"向量化进度: {progress_info['percentage']:.1f}% "
            ...
            logger.info(f"向量化完成: 成功处理 {total_added}/{len(documents)} 个文档")
            return vectorstore

        except Exception as e:
            logger.error(f"创建文档嵌入失败: {e}")
            return None
    ...
    def add_documents(self, documents: List[Document]) -> bool:
        ...
        try:
            ...
            logger.info(f"成功添加 {len(new_documents)} 个新文档")
            return True

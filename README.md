...
基于 LangChain 和 Gemini 的现代化 RAG 系统，采用分层架构设计，经过5个阶段的重构，实现了企业级的可维护性、可扩展性和可靠性。支持本地 PDF 文档读取、Web UI 展示，会话持久化，并可部署到 Hugging Face Spaces。
...
**重构升级 (v4.0)**: 经过5个阶段的重构，实现了企业级分层架构设计 + 内存管理服务 + 依赖注入容器 + 完整的基础设施层，替代单体应用架构，大幅提升可维护性、可扩展性和可靠性。
